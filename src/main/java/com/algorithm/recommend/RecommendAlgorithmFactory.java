...
public class RecommendAlgorithmFactory {
    ...
    public static class ModelLoader {
        ...
        public List<Long> recommendItems(Long userId, int numRecommendations, boolean useUserItemMatrix) {
            return recommendItems(userId, numRecommendations, useUserItemMatrix, true);
        }
         ...
         * @param useUserItemMatrix 是否使用协同过滤补充
         * @param useModelOnly 是否只使用模型（true=只使用随机森林模型，不使用协同过滤）
         */
        public List<Long> recommendItems(Long userId, int numRecommendations, boolean useUserItemMatrix, boolean useModelOnly) {
            ...
            if (useModelOnly) {
                ...
                System.out.println("最终推荐了 " + modelRecommendations.size() + " 个物品（仅模型）");
